...
import { sepoliaChainId } from "@/lib/contracts/config";
...
export function NetworkGuard() {
  ...
  if (!isConnected) {
    return (
      <section className="rounded-lg border border-slate-800 bg-slate-950 p-5">
        ...
        <p className="mt-2 text-sm text-slate-400">Sepolia status appears after wallet connection.</p>
      </section>
    );
  }
  ...
  if (chainId === sepoliaChainId) {
    return (
      <section className="rounded-lg border border-emerald-500/40 bg-emerald-950/30 p-5">
        ...
        <h2 className="mt-2 text-lg font-semibold text-white">Sepolia connected</h2>
  ...
  return (
    <section className="rounded-lg border border-amber-400/40 bg-amber-950/20 p-5">
      ...
      <h2 className="mt-2 text-lg font-semibold text-white">Switch to Sepolia</h2>
      <p className="mt-2 text-sm text-amber-100">
        Connected chain ID {chainId}. This app reads the casino contract on Sepolia only.
      </p>
      <button
